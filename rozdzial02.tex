...
% TO DO: Dokoñczyæ ogólny opis miejsca wdro¿enia (system operacyjny, infrastruktura)
Sama aplikacja wdro¿ona powinna byæ w~systemie ...
...
% ze strony https://cmmorrow.medium.com/using-sqlalchemy-and-flask-to-build-a-simple-data-driven-web-app-17e2d43778bb
...
\begin{itemize}[leftmargin=1cm]
  \item Wydajnoœæ: aplikacja powinna obs³u¿yæ ... u¿ytkowników (w tym ... pracuj¹cych równoczeœnie). Czas reakcji powinien byæ akceptowalny, choæ nie jest specjalnie istotny. 
...
\subsection{Wykorzystany framework i biblioteki} % TO DO: o narzêdziach pisze Pan w osobnym rozdziale
Do budowy aplikacji webowej zdecydowano siê u¿yæ frameworka Flask. Wykorzystano te¿ dodatkowe biblioteki, klasy, funkcje itp.\, co widaæ na listingu~\ref{lst:Python biblioteki}. U¿yto, miêdzy innymi, bibliotekê \texttt{flask\_mysqldb} zapewniaj¹c¹ po³¹czenie miêdzy Flask a MySQL. Klasa \texttt{MySQLdb.cursors} pozwala wykonywaæ i zapisywaæ wynik zapytania SQL w zmiennej typu s³ownik.
...
Ka¿dorazowo przy uruchamianiu aplikacji wykonywany jest poni¿szy kod (patrz na listing~\ref{lst:Podstawa programu}). Buduje on strukturê programu, aby w kolejnym kroku uruchomiæ go pod adresem \texttt{localhost} i na porcie o numerze \texttt{4449}.
%\newpage
...
Aby mo¿na by³o skorzystaæ z bazy danych utworzono do niej po³¹czenie (patrz na listing~\ref{lst:Konfiguracja polaczenia bazy danych i aplikacji}).
...
% TO DO: proszê zwróciæ uwagê na literówki: przegieblo 
...
Funkcja ka¿dorazowo wykonuje siê po uruchomieniu programu. Odpowiada ona za ³adowanie siê strony g³ównej oraz wyœwietlanie wszystkich wpisów z encji \texttt{Oferta}, je¿eli istnieje aktywna sesja u¿ytkownika (patrz na listing~\ref{lst:home}).
