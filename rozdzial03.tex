...
Do stworzenia bazy danych zastosowano trzy modele: uproszczony model konceptualny, rozszerzony model konceptualny (inaczej model logiczny) oraz model fizyczny. W celu unikniêcia nieœci³oœci obrano jeden standard modelowania. Przyjêto notacjê Crow's Feet, która jest pokrewna do IE/Relational. Opieraj¹c siê na nich stworzono i wygenerowano bazê.
...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.5]{Rys01/UproszczonyModelKonceptualny.png}
...
Korzystaj¹c z uproszczonego modelu konceptualnego stworzono jego rozszerzon¹ wersjê (patrz na rysunek~\ref{fig:Rys02/ModelLogiczny.png}). Tabele na wspomnianym rysunku posiadaj¹ dwie kolumny. W lewej wypisywane s¹ klucze. W prawej znajduj¹ siê nazwy indeksów. Ka¿da encja posiada klucz g³ówny (Primary key). Encje \textit{U¿ytkownik} i \textit{Oferta} posiadaj¹ kolejno klucze g³ówne o nazwie 'ID\textunderscore u¿ytkownika' oraz 'ID\textunderscore oferty'. Encja \textit{Pesonalia} dziedziczy klucz g³ówny od encji \textit{U¿ytkownik}, który jest zarazem kluczem obcym (Foreign key). Ponad to encja \textit{Oferta} dziedziczy g³ówny klucz encji \textit{U¿ytkownik} jako klucz obcy. Pod kluczami wypisano wszystkie indeksy jakie nale¿¹ do poszczególnych encji.
\begin{figure}[h!]
\centering
...
Opieraj¹c siê na rozszerzonym modelu konceptualnym sporz¹dzono model fizyczny (patrz na rysunek~\ref{fig:Rys03/ModelFizyczny.png}). Model ten, w przeciwieñstwie do wczeœniejszych, nie posida polskich znaków. Encje maj¹ po dwie kolumny: lewa zawiera nazwy indeksów, oznaczenia kluczy g³ównych i obcych, natomiast w prawej kolumnie okreœlono cechy poszczególnych indeksów. Znaczenia u¿ytych skrótów wypisano poni¿ej: 
\begin{itemize}[leftmargin=1cm]
...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.2]{Rys04/Success.png}
