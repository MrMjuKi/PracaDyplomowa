...
Obecny rozdzia³ skupia siê na zmianach dokonanych w aplikacji, które zwiêksz¹ jej bezpieczeñstwo. Poprawki w kodzie wynikaj¹ z analizy podsumowania przeprowadzonego audytu we wczeœniejszej czêœci pracy.

\section{Limit prób logowañ}
W celu ograniczenia du¿ej iloœci ¿¹dañ pojawiaj¹cych siê w krótkim zakresie czasu na stronie logowania zdecydowano siê zadeklarowaæ limity. U¿yto do tego funkcji z biblioteki \texttt{flask\_limiter} i \texttt{flask\_limiter.util} jak na listingu~\ref{lst:limitLogowan}. Skonfigurowanie wymaga³o zadeklarowania globalnej informacji, jakiej aplikacji oraz adresu IP tyczy siê limit (lis.~\ref{lst:globalLimiter}). Dla podstrony logowanie skonfigurowano maksymalnie piêæ logowañ na godzinê (lis.~\ref{lst:logowanieLimit}). Dodatkowo dla b³êdu 429 ustawiono informacjê ''Przekroczyles limit zadan'' (lis.~\ref{lst:429error}).
...
\begin{lstlisting}[language=Python, caption=Skonfigurowanie informacji przy b³êdzie 429, label={lst:429error}, basicstyle=\footnotesize\ttfamily]
...
\begin{lstlisting}[language=Python, caption={Skonfigurowanie walidaji pola login, e-mail i has³o}, label={lst:walidacjeRejestracja}, basicstyle=\footnotesize\ttfamily]
...
\begin{lstlisting}[language=Python, caption={Weryfikacja loginu i e-mailu podczas rejestracji}, label={lst:weryfikacja}, basicstyle=\footnotesize\ttfamily]
...
\begin{lstlisting}[language=Python, caption={Sprawdzanie jakoœci i cieniowanie has³a}, label={lst:jakoscICieniowanieHasla}, basicstyle=\footnotesize\ttfamily]
...
\section{Implementacja protoko³u HTTPS}
%Zastanawiam siê implementcj¹